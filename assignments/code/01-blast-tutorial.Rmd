...
To generate the API URL used with `curl` to successfully download the protein database for annotating blast results following go to [this Uniprot website link](https://www.uniprot.org/uniprotkb?facets=reviewed%3Atrue&query=%2A). Then click download > change format to TSV > click Generate URL for API > copy the link for API URL using the streaming endpoint. The link has already been placed in the following code chunk. 
...
* `head -2` prints the first two lines of the file
* `wc` counts the number of lines, words, and bytes in a file, and the -l flag specifies that it should count only the number of lines
...
The Blast table needs some reformatting to join with the annotation table...

This code uses the `tr` replaces the vertical bars (|) in the file located at ../output/Ab_4-uniprot_blastx.tab with tabs (\t). The modified file is then piped to head -2 to print the first two lines of the file with the replaced separator
